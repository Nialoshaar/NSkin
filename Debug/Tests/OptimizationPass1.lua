...
eq(counts.skin, 1, "changed registration skins once")
...
eq(counts.placement, nil, "appearance refresh skips saved placement")
clear()
...
clear()
N:SetWindowAppearanceOverride("Collections", "button.borderSize", 2)
eq(counts.module, 1, "window module refresh")
eq(counts.options, nil, "window skips options")
eq(counts.tabs, nil, "window skips global tabs")
clear()
N:SetAppearanceOverride("button.borderSize", 4)
eq(counts.skin, 1, "shared type refresh")
eq(counts.options, 1, "shared type refreshes options")
eq(counts.module, nil, "shared type skips modules")
eq(counts.tabs, nil, "shared type skips global tabs")
